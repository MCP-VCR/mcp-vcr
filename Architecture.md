...
*   **Type Confusion**: Swaps payload field data types (e.g., `str` → `42`, `int` → `"not_a_number"`, `list` → `dict`, `dict` → `list`, `bool` → `"true"`).
*   **Boundary Violations**: Replaces argument values with edge values (integers `0`, `-1`, `2**53`, `-(2**53)`; strings `""`, `1MB string`; lists `[]`, `10,000 items`; dicts `{}`, depth 100 nested object).
*   **Truncated Payloads**: Bypasses JSON serialization to send malformed raw bytes (50% sliced payload, opening brace `{`, empty bytes `b""`, trailing garbage `}}}}`).
