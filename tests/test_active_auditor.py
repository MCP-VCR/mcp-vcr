...
class MockReflectionAndExecutionTransport(MockTransport):
    async def write_to_server(self, data: bytes):
        try:
            req = json.loads(data.decode("utf-8").strip())
        except Exception:
            ...
            resp = {
                ...
                "result": {
                    "content": [
                        {
                            "type": "text",
                            "text": f"Input: {val} Output: MCP_VCR_CANARY_PROMPT_read_file_path",
                        }
                    ]
                },
            }
...
@pytest.mark.asyncio
async def test_active_audit_engine_reflection_and_execution_vulnerable():
    def transport_factory():
        return MockReflectionAndExecutionTransport()
