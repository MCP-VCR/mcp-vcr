...
class MockExecutionTransport(MockTransport):
    async def write_to_server(self, data: bytes):
        try:
            req = json.loads(data.decode("utf-8").strip())
        except Exception:
            ...
            resp = {
                ...
                "result": {
                    "content": [
                        {
                            "type": "text",
                            "text": "MCP_VCR_CANARY_PROMPT_read_file_path",
                        }
                    ]
                },
            }
...
@pytest.mark.asyncio
async def test_active_audit_engine_execution_vulnerable():
    def transport_factory():
        return MockExecutionTransport()
...
class MockPartialReflectionTransport(MockTransport):
    async def write_to_server(self, data: bytes):
        try:
            req = json.loads(data.decode("utf-8").strip())
        except Exception:
            ...
            resp = {
                ...
                "result": {
                    "content": [
                        {
                            "type": "text",
                            "text": "Invalid input for parameter 'path': contains prohibited marker MCP_VCR_CANARY_PROMPT_read_file_path",
                        }
                    ]
                },
            }
...
@pytest.mark.asyncio
async def test_active_audit_engine_partial_reflection_inconclusive():
    def transport_factory():
        return MockPartialReflectionTransport()
